...
| 참가비 | 20,000원 (반환 없음) |
| 예치금 | 100,000원 |
| 환급 | 주 6일 이상 인증 성공 + 주간 미션 달성 시 25,000원 |
| 성장 축 | 아비투스 7자본 중 3개 선택 + 공통과제 3종 |

**아비투스 7자본**: 심리 · 문화 · 지식 · 경제 · 신체 · 언어 · 상징
**공통과제 3종**: 독서 · 공부 · 주간 회고 (전원 필수, 회고는 점수 미반영)
...
| 운영진 (ADMIN) | `users.role = 'ADMIN'` | 참가자 전체 + 환급·MVP·지원서 |
...
- 내가 되고 싶은 인간상 / 외적 페르소나 / 내가 살고 싶은 삶 입력
...
공통과제 중 하나로, **주 1회만** 작성합니다. 같은 주에 다시 저장하면 그 주 회고가
갱신되고 새 카드가 쌓이지 않습니다.

1. 이번 주 회고
2. 다음 주 계획

주간 회고는 점수 계산에 포함되지 않습니다.
...
| 탭 | 기능 |
| --- | --- |
| 참가자 (기수별 주간 관리) | 주간 성공일수, 요일별 이행, 주간 미션 달성, 환급 대상 판정, 지급 체크, MVP 지정, CSV 내보내기 |
...
**WEEKLY 미션은 일별 판정에서 제외**합니다. 누적 방식이라 목표를 채우기 전에는 매일
감점되고, 채운 뒤에는 아무것도 안 해도 매일 완료로 잡히기 때문입니다. 대신 주 단위로
한 번만 달성 여부를 보고 환급의 별도 조건으로 씁니다.

```
환급 대상 = 성공일수 6일 이상  AND  그 주 WEEKLY 미션 전부 달성
```

WEEKLY 미션을 하나도 설정하지 않은 참가자는 두 번째 조건이 자동 통과됩니다.
자동 판정은 참고용이고, 실제 지급 여부는 운영진이 확인 후 직접 체크합니다.
...
- MVP 보너스의 대시보드 점수 자동 반영 (점수 보정 기능은 2026-08 제거)
- 완주 환급(4주 전액) 자동 계산
- 초대장 랜딩 페이지
